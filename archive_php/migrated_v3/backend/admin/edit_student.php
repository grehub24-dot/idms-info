            ...
            <div class="card">
                <form action="" method="POST" enctype="multipart/form-data" style="display:grid; grid-template-columns: 1fr 1fr; gap:20px;">
                    ...
                    <div style="grid-column: span 2; text-align: center; margin-bottom: 10px;">
                        <?php
                            $currentProfilePath = $student['profile_picture'] ?? '';
                            $currentProfileSrc = '../images/infotess.png';
                            if (!empty($currentProfilePath)) {
                                $rootCurrentProfile = __DIR__ . '/../../' . ltrim($currentProfilePath, '/');
                                $legacyCurrentProfile = __DIR__ . '/../' . ltrim($currentProfilePath, '/');
                                if (is_file($rootCurrentProfile)) {
                                    $currentProfileSrc = '../' . ltrim($currentProfilePath, '/');
                                } elseif (is_file($legacyCurrentProfile)) {
                                    $currentProfileSrc = '../backend/' . ltrim($currentProfilePath, '/');
                                }
                            }
                        ?>
                        <img src="<?php echo htmlspecialchars($currentProfileSrc, ENT_QUOTES, 'UTF-8'); ?>" alt="Current Profile" style="width: 120px; height: 120px; border-radius: 50%; object-fit: cover; border: 2px solid #ddd; margin-bottom: 10px;">
                        <br>
