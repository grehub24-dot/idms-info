...
<body>
        ...
        <main class="main-content">
            <div class="top-bar">
                ...
                <div style="display:flex; gap:10px;">
                    <button id="checkSmsConfigBtn" class="btn-primary" style="background:#6f42c1; border-color:#6f42c1;"><i class="fas fa-heartbeat"></i> Check SMS Config</button>
            ...
            <div id="sms-config-result" style="display:none;"></div>
    ...
    <script>
        const smsConfigBtn = document.getElementById('checkSmsConfigBtn');
        const smsConfigResult = document.getElementById('sms-config-result');

        smsConfigBtn.addEventListener('click', async () => {
            smsConfigBtn.disabled = true;
            smsConfigBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Checking...';
            try {
                ...
                const alertClass = data.ok ? 'alert alert-success' : 'alert alert-danger';
                const logText = data.last_log ? data.last_log : 'No SMS logs yet.';
                smsConfigResult.className = alertClass;
                smsConfigResult.style.display = 'block';
                smsConfigResult.innerHTML = `<strong>SMS Health:</strong> ${data.ok ? 'Ready' : 'Configuration issue detected'}<br>
                    <strong>Failed checks:</strong> ${failedText}<br>
                    <strong>Endpoint:</strong> ${data.settings?.endpoint || 'N/A'}<br>
                    <strong>Last log:</strong> ${logText}`;
            } catch (err) {
                smsConfigResult.className = 'alert alert-danger';
                smsConfigResult.style.display = 'block';
                smsConfigResult.textContent = 'Failed to run SMS health check.';
            } finally {
                smsConfigBtn.disabled = false;
                smsConfigBtn.innerHTML = '<i class="fas fa-heartbeat"></i> Check SMS Config';
            }
        });
